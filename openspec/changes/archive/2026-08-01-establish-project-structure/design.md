...
- **Spec debt accumulates and is never paid** → This is an informal project, so there is deliberately no reconciliation cadence, ritual, or owner: debt is cleared opportunistically. That removes every mitigation except one, which therefore has to carry the whole load — the `spec-debt` backlog must stay **countable**, so the drift is visible even when nobody is scheduled to look. A backlog growing steadily over time is the signal to act, either by narrowing the zone boundaries or by adopting a deliberate pass after all.
