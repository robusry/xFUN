...
Broadcast availability SHALL be modeled and served separately from scoring, with its own update cadence, and SHALL be able to express that availability is unknown. The system SHALL NOT assert an availability answer it cannot substantiate. Availability SHALL be served from stored data rather than returned as a fixed value, and SHALL NOT appear in the match snapshot a model receives, so that no model can score a match differently according to who broadcasts it.
